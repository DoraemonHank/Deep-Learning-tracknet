...
#python train_TrackNet.py --save_weights=myModel2/model --dataDir=npy --epochs=300 --tol=5 --load_weights=pretrain/model_best
#python predict.py --video_name=test/2_15_08.mp4 --load_weights=pretrain/model_best --label=test/2_15_08_ball.csv --tol=5
...
if paramCount['load_weights'] == 0:
	...
	model.compile(loss=['categorical_crossentropy'], optimizer=ADADELTA, metrics=['accuracy']) #loss = my_loss ['binary_crossentropy']
#Retraining
else:
	...
	print(load_weights)
	start_epochs = int(load_weights[load_weights.find('_')+1:])
